...
server <- function(input, output, session) {
  ...
  varnames <- c("Forest_loss", "Slope")
  axisbounds <- list(c(0.0001, 0.001, 0.01, 0.1, 1), c(0.0001, 0.001, 0.01, 0.1, 1))
  axislabels <- list(c('0.0001', '0.01', '1', '100', '10000'), c('0.0001', '0.01', '3', '100', '10000'))
  ylabs <- c("Forest loss (poportion of patch)", "Slope")
  
  # where does this go?
  output$plot2 <- renderPlot({
    plot(SHARP_patches_4plot$Saltmarsh_Sparrow_abundance[as.logical(bystate[,as.numeric(input$state)])], SHARP_patches_4plot[,varnames[as.numeric(input$ytest)]][as.logical(bystate[,as.numeric(input$state)])], log="xy", xaxt="n", yaxt="n", 
