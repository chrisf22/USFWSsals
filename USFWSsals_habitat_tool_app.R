...
body <- dashboardBody(
  ...
  tabItems(
    ...
    tabItem(tabName = "dashboard",
            fluidRow(column(6,div(style = "height:680px; width:550px; background-color: rgba(187, 187, 187, 0); overflow-y: scroll;padding: 0px; margin-left: 15px", 
                                  div(style = "font-size:15px; color: #6A6A6A", "Click on a point to see its location in the map window. Attributes for the mapped location are shown below the plot."), br(),
    ...
    tabItem(tabName = "background", 
            fluidRow(
              column(9, div(style = "padding: 15px;", 
                            div(style = "font-size:17px; color: #6A6A6A", "The goal of this application is to synthesize datasets relevant to the management of Saltmarsh Sparrows (", em("Ammospiza caudacuta"), ") that are difficult to access or would otherwise require specialized software to 
                                ...
                                marsh patch that is behind tidal restrictions (“Proportion of marsh behind tidal restriction”), and the rate of sea-level rise that the marsh is experiencing (“Sea-level rise rate”). "), br(),
                            div(style = "font-size:17px; color: #6A6A6A", HTML("Each marsh patch is shown as a clickable dot on the main plot. Once clicked, the location of a marsh patch will be displayed on the map and the attributes for the patch will be printed to a table below the plot. To begin, 
                                ...
                                lower rates would require less intensive thin layer management. The highlighted green points show locations that exceed the specified threshold for both the x and y variables – i.e. marshes that might be suitable for management in areas that would benefit Saltmarsh Sparrows.")), br(),
                            div(style = "font-size:17px;color: #6A6A6A", HTML("Saltmarsh Sparrow abundance estimates are from Wiest et al. (2018). Marsh patch boundaries follow Wiest et al. (2016), who used a 50-m buffering approach to group marshes into biologically relevant units using the U.S. Fish and Wildlife 
                                ...
                                (mm/year) is from the NOAA tide gauge that is closest to the marsh patch. For more information, see the downloadable documentation below.")), br(),
                            div(style = "font-size:17px; color: #6A6A6A", HTML("This app and its features, including all new analyses, were developed by Christopher R Field and funded in part by the U.S. Fish and Wildlife Service, Atlantic Coast Joint Venture (ACJV). To access the spatial files used in this app, see the"), tags$a(href="https://acjv.org/tools-and-data/", "ACJV data page.")), br(), br(),
                            downloadButton("downloadData", "Download documentation"), br(), br(), br(),
                            div(style="display: inline-block; margin-left: 0px", 
                                tags$a(rel="license", href="http://creativecommons.org/licenses/by-nc/4.0/", img(alt="Creative Commons License", style="border-width:0", src="https://i.creativecommons.org/l/by-nc/4.0/88x31.png")),
                                HTML("&nbsp", "This work is licensed under a"),
                                tags$a(rel="license", href="http://creativecommons.org/licenses/by-nc/4.0/", "Creative Commons Attribution-NonCommercial 4.0 International License.")), br())
              )
            )
    )
  ))

ui <- dashboardPage(header, sidebar, body, skin = "black", tags$head(tags$style(HTML(
  '.help-block a {color: black;}'))))
...
server <- function(input, output, session) {
  ...
  round_df <- function(df) {
    ...
    #df[,"SHARP Patch ID"] <- as.character(df[,"SHARP Patch ID"])
  ...
  output$table <- renderTable({if(identical(click_info2(), numeric(0))){message}else{
    round_df(nearPoints(SHARP_patches_4plot[as.logical(bystate[,as.numeric(input$state)]), ], xvar="Sparrow abundance", yvar=varnames[as.numeric(input$ytest)], input$plot2_click, addDist = TRUE, threshold = 10)[1 ,c("SHARP Patch ID", "Sparrow abundance", "Forest loss", "Prop. restricted", "SLR")])
  ...
  click_info2 <- reactive(
    nearPoints(SHARP_patches_4plot[as.logical(bystate[,as.numeric(input$state)]), ], xvar="Sparrow abundance", yvar=varnames[as.numeric(input$ytest)], input$plot2_click, addDist = TRUE, threshold = 10)[,"SHARP Patch ID"])
  
  message <- data.frame("No point selected")
  colnames(message) <- "  "
  output$click_info <- renderPrint({if(identical(click_info2(), numeric(0))){message}else{
    round(nearPoints(SHARP_patches_4plot[as.logical(bystate[,as.numeric(input$state)]), ], xvar="Sparrow abundance", yvar=varnames[as.numeric(input$ytest)], input$plot2_click, addDist = TRUE, threshold = 10)[,c("SHARP Patch ID", "Forest loss", "Sparrow abundance", "Prop. restricted", "SLR")], 2)
  }})
  ...
  output$plot2 <- renderPlot({
    par(mar=c(5, 4.3, 0, 3))
    plot(SHARP_patches_4plot[as.logical(bystate[,as.numeric(input$state)]), "Sparrow abundance"], SHARP_patches_4plot[,varnames[as.numeric(input$ytest)]][as.logical(bystate[,as.numeric(input$state)])], log=loglogical[as.numeric(input$ytest)], xaxt="n", yaxt="n", 
  ...
  output$mymap <- renderLeaflet({
    if(as.numeric(input$basemap)==1){
      ...
      leaflet(SHARP_polys2()) %>%
        #setView(lng=bounds()[1, 1], lat=bounds()[2, 1], zoom = 8)
        fitBounds(lng1 = bounds()[1, 1], lat1 = bounds()[2, 1], lng2 = bounds()[1, 2], lat2 = bounds()[2, 2], options = list(maxZoom = 12)) %>%
        addPolygons(color = "#444444", weight = 1, smoothFactor = 0.5,
                    opacity = 1.0, fillOpacity = 0.5) %>%
        #addPolylines() %>%
        addTiles()
      #fitBounds(SHARP_polys2(), zoom = 5)
      #addMarkers(data = latlong()) #%>%
      #fitBounds(bounds()[1, 1], bounds()[2, 1], bounds()[1, 2], bounds()[2, 2])
      #clearBounds()
      #groupOptions("SHARP polys", zoomLevels = 1:10)
    }
  })
